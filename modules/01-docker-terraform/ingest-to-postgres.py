...
# Commented to fit ELT pattern - leaving transformation after data is loaded to data lake
# def convert_datetime_field(df: pd.DataFrame) -> None:
#     """
#     for a general purpose pipeline, data transformation should be handled
#     """
...
def main(params):
    ...
    try:
        ...
        df_head.to_sql(name=params.table_name, con=postgres_engine, index=False, if_exists="replace", dtype=TEXT)
        ...
        for df_chunk in df_iter:
            ...
            df_chunk.to_sql(name=params.table_name, con=postgres_engine, index=False, if_exists="append", dtype=TEXT)
            t_end = time()
